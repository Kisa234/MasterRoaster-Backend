...
export class PaqueteDataSourceImpl implements PaqueteDataSource {
    async create(dto: CreatePaqueteDto, numero_correlativo: string): Promise<PaqueteEntity> {
    ...
    async actualizarEstado(
        ...
        const paquete = await tx.paquete.update({
            where: { id_paquete },
            data: { estado: estado as any, ...extra },
        });
        return PaqueteEntity.fromObject(paquete);
    }
    async getByPedidoOrigen(id_pedido: string): Promise<PaqueteEntity | null> {
        const paquete = await prisma.paquete.findFirst({
            where: { id_pedido_origen: id_pedido },
            orderBy: { fecha_registro: 'desc' },
        });
        if (!paquete) return null;
        return PaqueteEntity.fromObject(paquete);
    ...
    async getByIdConItems(id_paquete: string): Promise<PaqueteConItemsEntity | null> {
        const paquete = await prisma.paquete.findUnique({
            where: { id_paquete },
            include: { items: true, envios: true },
        });
        if (!paquete) return null;

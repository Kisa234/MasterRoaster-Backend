...
export class BalonGasController {
  ...
  constructor(private readonly balonGasRepository: BalonGasRepository) {}
  ...
  public startBalonGas = async (req: Request, res: Response) => {
    const user = (req as any).user;
    const [error, dto] = StartBalonGasDto.create({ ...req.body, id_user_inicio_uso: user.id_user });
    if (error) return res.status(400).json({ error });
  ...
  public finalizeBalonGas = async (req: Request, res: Response) => {
    const user = (req as any).user;
    const [error, dto] = FinalizeBalonGasDto.create({ ...req.body, id_user_fin_uso: user.id_user });
    if (error) return res.status(400).json({ error });
